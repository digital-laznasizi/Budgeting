...
# ==========================================
# 2. FUNGSI FORMAT ANGKA (RUPIAH & TITIK)
# ==========================================
def fmt_rp(val):
...
# ==========================================
# 4. SIDEBAR (FORM ISIAN)
# ==========================================
st.sidebar.header("⚙️ Form Input Parameter")
budget = st.sidebar.number_input("Budget Iklan (Rp)", value=50000000, step=1000000)
cpc = st.sidebar.number_input("Proyeksi CPC (Rp)", value=40, step=5)
ctr = st.sidebar.number_input("Target CTR (%)", value=6.5, step=0.1)
cvr = st.sidebar.number_input("Target CVR (%)", value=1.5, step=0.1)
avg_donasi = st.sidebar.number_input("Rata-rata Zakat/Donasi (Rp)", value=100000, step=10000)
...
# BARIS 1: Menampilkan angka yang diinput (Budget, CTR, CVR, Donasi)
...
col4.empty() # Dikosongkan agar rapi sesuai grid 4 kolom

st.write("") # Memberi sedikit jarak antar baris

# BARIS 2: Menampilkan hasil hitungan Reach, Klik, Donatur, Total Dana
...
with st.expander("📊 Lihat Visualisasi Corong Konversi (Funnel) Paid Ads"):
    ...
    # Styling agar transparan menyatu dengan tema dark/light mode Streamlit
    fig.update_layout(
